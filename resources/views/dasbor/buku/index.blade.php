...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        ...
        <div class="flex flex-col sm:flex-row gap-3">
            ...
            <a href="{{ route('dasbor.buku.create') }}" class="inline-flex items-center justify-center px-4 py-2 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4">
                                <div class="flex items-start">
                                    ...
                                    <div class="ml-4">
                                        <div class="text-sm font-medium text-gray-900 line-clamp-1" title="{{ $book->title }}">{{ $book->title }}</div>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex justify-end space-x-2">
                                    <a href="{{ route('dasbor.buku.edit', $book) }}" class="text-indigo-600 hover:text-indigo-900 p-1 hover:bg-indigo-50 rounded transition-colors" title="Edit">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                        </svg>
                                    </a>
                                    <form action="{{ route('dasbor.buku.destroy', $book) }}" method="POST" class="inline-block" onsubmit="return confirm('Apakah Anda yakin ingin menghapus buku ini?');">
                                        @csrf
                                        @method('DELETE')
                ...
                @forelse($books as $book)
                    <div class="p-4 space-y-3">
                        <div class="flex justify-between items-start">
                            ...
                            <span class="px-2 py-1 rounded-full text-xs font-medium {{ $book->available > 0 ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                Stok: {{ $book->available }}
                            </span>
                        </div>
                        ...
                        <div class="flex justify-end space-x-3 pt-2">
                            <a href="{{ route('dasbor.buku.edit', $book) }}" class="text-indigo-600 font-medium text-sm hover:text-indigo-800">Edit</a>
                            <form action="{{ route('dasbor.buku.destroy', $book) }}" method="POST" class="inline-block" onsubmit="return confirm('Hapus buku ini?');">
                                @csrf
                                @method('DELETE')
